...
public class OrderRequest {
    ...
    public int quantity;
    public String replyTo; // Topic to send reply to
    public String correlationId; // To correlate request with response
}
...
// 3. REQUEST SENDER (Producer that expects a reply)
...
// 4. REQUEST PROCESSOR (Consumer that sends reply)
package com.example.consumer;

import com.example.model.OrderRequest;
import com.example.model.OrderResponse;
import io.smallrye.reactive.messaging.kafka.KafkaRecord;
import io.smallrye.reactive.messaging.kafka.reply.KafkaReply;
import jakarta.enterprise.context.ApplicationScoped;
...
// 5. RESPONSE HANDLER (Consumer that receives replies)
package com.example.consumer;
...
// 6. REST ENDPOINT TO TRIGGER REQUEST
package com.example.resource;

import com.example.model.OrderRequest;
import com.example.model.OrderResponse;
import com.example.producer.OrderRequestSender;
import jakarta.inject.Inject;
...
mp.messaging.outgoing.order-requests-out.connector=smallrye-kafka
mp.messaging.outgoing.order-requests-out.topic=order-requests
mp.messaging.outgoing.order-requests-out.value.serializer=io.quarkus.kafka.client.serialization.ObjectMapperSerializer

# Request channel (incoming - for processing)
mp.messaging.incoming.order-requests-in.connector=smallrye-kafka
mp.messaging.incoming.order-requests-in.topic=order-requests
mp.messaging.incoming.order-requests-in.value.deserializer=io.quarkus.kafka.client.serialization.ObjectMapperDeserializer
mp.messaging.incoming.order-requests-in.value.deserializer.type=com.example.model.OrderRequest

# Response channel (outgoing - manual)
mp.messaging.outgoing.order-responses-out.connector=smallrye-kafka
mp.messaging.outgoing.order-responses-out.topic=order-responses
mp.messaging.outgoing.order-responses-out.value.serializer=io.quarkus.kafka.client.serialization.ObjectMapperSerializer

# Response channel (incoming - for handling replies)
mp.messaging.incoming.order-responses-in.connector=smallrye-kafka
mp.messaging.incoming.order-responses-in.topic=order-responses
mp.messaging.incoming.order-responses-in.value.deserializer=io.quarkus.kafka.client.serialization.ObjectMapperDeserializer
mp.messaging.incoming.order-responses-in.value.deserializer.type=com.example.model.OrderResponse

# Request-Reply configuration (built-in)
mp.messaging.outgoing.order-requests-out.reply.topic=order-responses
mp.messaging.outgoing.order-requests-out.reply.timeout=30000
*/
